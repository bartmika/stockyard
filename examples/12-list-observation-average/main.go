...
import (
	...
	model "github.com/bartmika/stockyard/internal/domain/observation_average"
...
func main() {
	...
	dto := &dtos.ObservationAverageFilterRequestDTO{
		EntityIDs:               []uint64{1},
		Frequency:               model.ObservationAverageDayFrequency,
	...
	res, err := client.ListObservationAverages(dto)
	if err != nil {
		log.Fatal(err)
	}
	...
	// Print the results.
	if res.Count > 0 {
		log.Println("the `average` results from the server are as follows:")
