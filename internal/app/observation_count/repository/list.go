...
func (s *ObservationCountRepoImpl) getWhereKeysByFilter(f *domain.ObservationCountFilter) sq.And {
...
func (s *ObservationCountRepoImpl) ListByFilter(ctx context.Context, f *domain.ObservationCountFilter) ([]*domain.ObservationCount, error) {
	...
	var arr []*domain.ObservationCount
	defer rows.Close()
	for rows.Next() {
	...
	if arr == nil {
		return []*domain.ObservationCount{}, nil
	}
	return arr, err
}
